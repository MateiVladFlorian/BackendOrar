...
namespace BackendOrar.Controllers
{
    ...
    public class CourseController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> GetCoursesAsync([FromForm] CourseFilterModel? model)
        {
            Course[] courses = await courseService.GetCourses(model);
            logger.LogInformation("Classroom entries count: {0}.", courses.Length);
            return Ok(courses);
        }

        [HttpPost("create"), Authorize]
        public async Task<IActionResult> AddCourseAsync([FromForm] CourseRequestModel? model)
        {
            ...
            var (status, course, message) = await courseService.AddCourse(accessToken, model);
            ...
            return status switch
            {
                ...
                0 => Conflict(new ApiResponse<Course>(409, message, course)),
                1 => Created(),
